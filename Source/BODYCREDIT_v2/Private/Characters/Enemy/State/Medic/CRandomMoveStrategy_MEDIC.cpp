...
void CRandomMoveStrategy_MEDIC::Move(ACNox_EBase* Owner, float DeltaTime)
{
	if (!Owner) return;
...
void CRandomMoveStrategy_MEDIC::HearingMove(ACNox_EBase* Owner)
{
...
void CRandomMoveStrategy_MEDIC::RandomMove(ACNox_EBase* Owner)
{
AAIController* AICon = Cast<AAIController>(Owner->GetController());
	if (!AICon) return;
...
FVector CRandomMoveStrategy_MEDIC::GetRandomLocation(const ACNox_EBase* Owner) const
{
