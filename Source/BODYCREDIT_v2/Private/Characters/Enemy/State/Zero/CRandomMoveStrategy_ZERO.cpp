...
void CRandomMoveStrategy_ZERO::Move(ACNox_EBase* Owner, float DeltaTime)
{
	if (!Owner) return;
...
void CRandomMoveStrategy_ZERO::ResetVal(ACNox_EBase* Owner)
{
	bMoving = false;
...
void CRandomMoveStrategy_ZERO::RandomMove(ACNox_EBase* Owner)
{
...
FVector CRandomMoveStrategy_ZERO::GetRandomLocation(const ACNox_EBase* Owner) const
{
...
void CRandomMoveStrategy_ZERO::HearingMove(ACNox_EBase* Owner)
{
	AAIController* AICon = Cast<AAIController>(Owner->GetController());
	if (!AICon) return;
